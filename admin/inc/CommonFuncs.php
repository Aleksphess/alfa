	...
	foreach ($tags as $tag) {
            if ($tagsGroups !== null) {
                if (in_array($tag['parent_id'],$issetKeys) || in_array($tag['alias'],$tagsGroups)) {
                    $relation_items[$tag['parent_id']][$tag['id']]=array(
                        'id'=>$tag['id'],
                        'echo_name'=>$tag['name'],
                        'table' => $table,
                        'sort' => isset($tagsAssoc[$tag['id']]['sort']) ? $tagsAssoc[$tag['id']]['sort'] : 0
                    );
                    $issetKeys[] = $tag['id'];
                }    
            } else {
...
function buildTreeList($items, $savedValues){	
	...
	foreach($items as $item){
		...
		if(isset($item['items'])){
			...
			$menu.= '<li>'.$item['echo_name'].'<input type="checkbox" name="tags_values[]" value="'.$item['id'].'" '.$chk.'/>';
                        if ($item['table'] == 'brands') {
                            $menu.='<br/><input type="number" name="tags_sort['.$item['id'].']" value="'.$item['sort'].'" />';
                        }
                        $menu.='</li>';
		}
	}
   ...
   if (!empty($_POST['tags_values'])) 
    foreach ($_POST['tags_values'] as $value) {
            ...
            if (!empty($value)) {
                ...
                if (!empty($_POST['tags_sort']) && !empty($_POST['tags_sort'][$value])) {
                    $qi.= ", sort = " . $_POST['tags_sort'][$value];
                } 
                pdoExec($qi);
